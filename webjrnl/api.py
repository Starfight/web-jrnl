...
class EntriesApi(Resource):
    def get(self, timestamp_start=None, timestamp_end=None):
        # parse date
        date_start = datetime.now().replace(hour=0, second=0, minute=0, microsecond=0)
        date_end = date_start.replace(hour=23, second=59, minute=59, microsecond=999999)
        try:
            if timestamp_start:
        ...
        # search entries
        default_journal.filter(start_date=date_start.isoformat(), end_date=date_end.isoformat())
        return jsonify({"entries": [x.to_dict() for x in default_journal.entries]})
